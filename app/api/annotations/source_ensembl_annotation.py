...
def getENSEMBLfromDB(ensembleid):
    ...
    try:
        ...
        if (len(query)) == 0:
            query = {
                "error": f"{ensembleid} not found in DB",
            }
...
def _process_response_from_ENSEMBL(data: dict, id: str) -> dict:
    ...
    for element in data:
        if str(element["feature_type"]) == "transcript" and element["Parent"] == id:
            ...
            transcript[element["transcript_id"]] = {
                "external_name": "-",
        ...
        out["transcripts"][type][name].append(
            {"x": element["start"], "y": element["end"]}
        )
    if flag:
...
def getENSEMBLannotations(request, ensemblid: str):
    ...
    if "error" in ensembl:
        ...
        if "error" in data:
            ...
            return HttpResponse(
                json.dumps(
                    {
                        "error": f"{ensemblid} not found in DB",
                    }
                ),
    ...
    if result:
        ...
        status_code = 404
    return HttpResponse(
        json.dumps(result), content_type="application/json", status=status_code
    )
